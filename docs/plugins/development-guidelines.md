...
- **The menu bar panel and widgets** use `PluginPanelTheme.Typography` and `PluginPanelTheme.Symbol`, a fixed compact scale of whole points (13 / 12 / 11 / 10 / 9 under a 14 pt heading) that keeps menu-like rows stable and keeps every step visibly different from its neighbors. Hierarchy is weight and size as a set: one semibold row title, a medium description one step below, semibold badges. Symbols share the same steps so an icon never outweighs its label.
...
**Form before color.** Decide what the data's job is before reaching for a slot. A single current value with a trend is a stat tile, and a ratio against a limit is a meter; both wear `primary` (the sparkline included), or the status scale when the fill itself means severity, such as a battery running low. Never give each metric its own hue: CPU, GPU, and memory tiles are three stat tiles, not three series of one chart. Paired series such as read/write or down/up take `primary` and `secondary`, so the second color means the same thing in every widget. Only a chart whose series are the subject, such as Activity Bar's keys, clicks, and scrolls, goes further, assigned in sequence with a legend from two series on; three is the practical ceiling before the widget needs small multiples. Values, labels, and legends wear `theme.text` tokens with a colored dot or line beside them, never the series color.
