...
- Type sizes and corner radii are tokens, never literals: settings and floating cards use `PluginSettingsTheme.Typography` (semantic, user-scalable) and menu bar panel rows and widgets use the compact fixed scale in `PluginPanelTheme.Typography` / `PluginPanelTheme.Symbol`; every radius comes from `PluginSettingsTheme.Radius` (`chip`, `field`, `control`, `card`, `hostCard`, `overlay`), with `nested(_:inset:)` keeping inset shapes concentric and `appIcon(for:)` for app-icon clips. Miniature previews may keep their own reduced sizes.
- Interaction colors are named roles, never inline opacities: hover, press, selection, emphasis, and tinted icon tiles use the wash scale in `PluginSettingsTheme.Palette` (`chipBackground`, `hoverBackground`, `materialHoverBackground`, `pressedBackground`, `emphasisBackground`, `selectionBackground`, `tintBackground(_:)`), rings use `hairlineBorder` / `subtleBorder` / `hoverBorder` / `selectionBorder` / `contrastBorder`, focus uses `focusRing`, and palettes use `scrim`; menu bar panel content keeps the theme's `surfaces` and `interaction` palettes.
- Chart and widget colors come from `PluginComponentTheme.dataSeries` and `theme.status` only: categorical slots are assigned in fixed order with `color(at:)` (never cycled; extras wear `other`), magnitude uses the one-hue `sequential` ramp, polarity uses the diverging trio, and status colors are reserved for state with an icon or label. Do not hard-code `systemBlue`-style series colors in widgets; brand colors and feature thresholds remain plugin concerns.
- Pick the form before the color. A single value with a trend is a stat tile and a ratio against a limit is a meter; both wear `primary` (the status scale only when the fill means severity), never a hue per metric. Paired series such as read/write or down/up take `primary` and `secondary`; only a chart whose series are the subject goes further, in sequence, with a legend from two series on. Values, labels, and legends wear `theme.text` tokens with a colored mark beside them, never the series color.
